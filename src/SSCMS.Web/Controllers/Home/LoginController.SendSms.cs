...
namespace SSCMS.Web.Controllers.Home
{
    public partial class LoginController
    {
        ...
        public async Task<ActionResult<BoolResult>> SendSms([FromBody] SendSmsRequest request)
        {
            ...
            if (!SmsRateLimitUtils.TryConsume(_cacheManager, typeof(LoginController), mobile, PageUtils.GetIpAddress(Request), out var retryAfterSeconds))
            {
            ...
            (success, errorMessage) =
                await _smsManager.SendSmsAsync(mobile, SmsCodeType.LoginConfirmation, code);
            if (!success)
            {
                return this.Error(errorMessage);
            }
