...
namespace SSCMS.Web.Controllers.Home
{
    public partial class RegisterController
    {
        ...
        public async Task<ActionResult<BoolResult>> SendSms([FromBody] SendSmsRequest request)
        {
            ...
            if (!SmsRateLimitUtils.TryConsume(_cacheManager, typeof(RegisterController), mobile, PageUtils.GetIpAddress(Request), out var retryAfterSeconds))
            {
            ...
            var (success, errorMessage) =
                await _smsManager.SendSmsAsync(mobile, SmsCodeType.Registration, code);
            if (!success)
            {
